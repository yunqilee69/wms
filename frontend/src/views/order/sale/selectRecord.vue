  ...
  <el-dialog title="选择库存" v-model="visible" width="800px" top="5vh" append-to-body>
    ...
    <el-row>
      <el-table @row-click="clickRow" ref="refTable" :data="recordList" @selection-change="handleSelectionChange" height="260px">
        ...
        <el-table-column label="货物名称" align="center" key="wareName" prop="wareName" />
...
<script setup>
import { getUnSaleRecordList, addUnSaleRecord } from "@/api/order/sale"
...
// 查询表数据
function getList() {
  getUnSaleRecordList(queryParams).then(res => {
...
function handleSelectRecord() {
  ...
  addUnSaleRecord({ orderId: orderId, recordIds: uIds, detailType: props.detailType }).then(res => {
