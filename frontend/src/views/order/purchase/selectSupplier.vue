<template>
  <!-- 授权用户 -->
  <el-dialog title="选择供应商" v-model="visible" width="800px" top="5vh" append-to-body>
    <el-row :gutter="20">
      <!--供应商数据-->
...
<script setup>
import { getSupplierList } from "@/api/user/supplier.js"
import { addOrder } from "@/api/order/purchase.js"
...
const handleCurrentChange = (val) => {
  ...
  supplierId.value = val.id
}

// 获取数据
function getList() {
  getSupplierList(queryParams).then(res => {
...
function handleSelectRecord() {
  ...
  addOrder(supplierId.value).then(res => {
